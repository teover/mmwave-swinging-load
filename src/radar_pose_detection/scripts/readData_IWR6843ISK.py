        ...
        for tlvIdx in range(numTLVs):
            ...
            if tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS:
                ...
                for objectNum in range(numDetectedObj):
                    ...
                    if closestValue > 0.6 or closestValue < 0.1:
                        bestX = bestX
                    else:
                        bestX =  closestValue
                    ...
                    # CSV logging
                    fields = [rospy.Time.now(), x1, x2, x3, closestValue, bestX, usedXrad]

                    with open(r'6843_data_log.csv', 'a') as f:
                        writer = csv.writer(f)
                    ...
                    # msg.x-1 : 1 meter is subtracted as the pile is placed 1 meter from the radar, length of pendulum is 1 meter
                    command_to_joint_stateX.joint_state.position = (0, bestX, 0)

                    command_to_joint_stateX.joint_state.header.stamp = rospy.Time.now()
                    # Publish to topic
                    command_to_joint_stateX.joint_pub.publish(command_to_joint_stateX.joint_state)
